...
@Component({
  selector: 'app-ap-district-list',
  templateUrl: './ap-district-list.component.html',
  styleUrls: ['./ap-district-list.component.css']
})
export class ApDistrictListComponent implements OnInit {
  ...
  ngOnInit() {
    const overViewYear = this._overview.fetchOverviewFinalAp();
    const overViewYes = this._overview.fetchOverviewFinalDailyAp();
    const overViewWeek = this._overview.fetchOverviewFinalWeeklyAp();
    ...
    forkJoin([overViewYear, overViewYes, overViewWeek]).subscribe(results => {
      if (!results[0].hasOwnProperty('results') || !results[1].hasOwnProperty('results')) {
        ...
        this.dataSourceWeekly = new MatTableDataSource<any>(results[1].results);
      }
    });
  }
  ...
  invokePaginator(event) {
    ...
    if (event.value === 'week') {
      this.dataSourceWeekly = new MatTableDataSource<any>(this.dataSourceDailyReal);
      return;
    }
  }
  ...
  navigate(row) {
    this._router.navigate(['/dashboards/social-media/ap-district', row.page_id]);
  }

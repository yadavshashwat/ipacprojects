			...
			if obj["type"] == "call":
				...
				if obj["command"] == "backend-connectWhatsApp":
					...
					if cmd == "backend-generateQRCode":
						...
						currWhatsAppInstance.relogin(callback);
...
server = SimpleWebSocketServer("", 2023, WhatsAppWeb);
eprint("whatsapp-web-backend listening on port 2022");
server.serveforever();

...
//                 redirect('vote');
    ...
    public function index(){
		...
		$data['referral_code']=$this->user_leader->getReferralCode($user_session_id);
        //echo '<pre>';print_r($data);exit;
        $this->load->view('register',$data);

        $bro_ip= $_SERVER['REMOTE_ADDR'];

//echo $bro_ip;
$details = json_decode(file_get_contents("http://ipinfo.io/{$bro_ip}/json?token=40a7a63805399b"));
//echo $details->city;
$city=$details->city;
//echo $details->region;
$state=$details->region;
$sql_ip = "INSERT INTO `ip_city_state` (`SNO`, `IP`, `CITY`, `STATE`) VALUES ('','$bro_ip', '$city', '$state')";           $query = $this->db->query($sql_ip);
    ...
    public function submit_register()
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $collage_id = 0;
                $collage_name = trim(str_replace("'", "", $this->input->post('regCollName')));
                ...
                $params['collage_name'] = trim(str_replace("'", "", $this->input->post('regCollName')));
                ...
                $params['language_preference_1'] = $this->input->post('regLangPreference1');
                $params['language_preference_2'] = $this->input->post('regLangPreference2');
                $params['is_active'] = 1;
                $params['is_pta'] = 1;
                ...
                if (!empty($user_details)) {
                    ...
                    if ($status == "Y") {
                        ...
                        if($langcookie=="hi"){
                            $email_data['subject'] = "I-PAC के साथ PTA के तौर पर रजिस्ट्रेशन";
                        }else{
                            $email_data['subject'] = "Registration as a Part Time Associate of I-PAC";
                        }
                        $email_data['email_view'] = "emails/register_thankyou";
                        $email_result = $this->send_mail->htmlmail($email_data);

                        if($langcookie=="hi"){
                            $message = "इडियन पॉलिटिकल एक्शन कमिटी (I-PAC) के राष्ट्र के लिए अभियान के साथ पार्ट टाइम एसोसिएट (PTA) के तौर पर रजिस्ट्रेशन करने के लिए आपका धन्यवाद। आपका यूनीक रजिस्ट्रेशन आईडीः " . $user_details->registration_number."  हहै। राष्ट्र के लिए अभियान से जुड़े अपडेट के लिए हमसे फेसबुक, ट्विटर एवं इंस्टाग्राम पर जुड़ें।";
                        }else{
                            $message = "Thanks for registering as a Part Time Associate (PTA) to Campaign for India with the Indian Political Action Committee (I-PAC). Your unique registration ID is " . $user_details->registration_number."  Connect with I-PAC on Facebook, Twitter, and Instagram for Campaign for India updates.";
                        }


                        $this->register->send_otp($user_details->mobile_number, $message);
                        $this->register->send_IVR($user_details->mobile_number);

                        setcookie("register_complete","1",time() + (10 * 365 * 24 * 60 * 60),"/");
    ...
    public function verify_otp(){
        ...
        if(!empty($_POST)){
            ...
            $user_otp = $this->input->post('user_otp_regi');
            $user_id = $_SESSION['user']['id'];
            if($user_id){
                    ...
                    if($user_otp == $user_details->otp && strtotime($user_details->otp_generate_timestamp) < strtotime('+30 minutes',strtotime(Date("Y-m-d H:i:s")))){
                        ...
                        if($update_result){
                            ...
                            if($user_details->is_pta == 0){
                                ...
                                if($langcookie=="hi"){
                                    $email_data['subject'] = "पीटीए पुष्टिकरण और पहला काम";
                                }else{
                                    $email_data['subject'] = "PTA Confirmation and First Task";
                                }
                                $email_data['email_view'] = "emails/register_thankyou";
                                $email_result = $this->send_mail->htmlmail($email_data);
                                ...
                                if($langcookie=="hi"){
                                    $message = "पार्ट टाइम एसोसिएट (पीटीए) के तौर पर रजिस्ट्रेशन करने के लिए आपका धन्यवाद। आपका युनीक वोटिंग लिंकः http://www.indianpac.com/naf/referral/" . $user_details->registration_number."  है। लोगों के साथ इसे शेयर कर, NAF में वोट दिलाएं।";
                                }else{
                                    $message = "Thanks for registering as a Part Time Associate (PTA). Your unique voting link is: http://www.indianpac.com/naf/referral/" . $user_details->registration_number." Share it with people to get votes on NAF.";
                                }
                                $this->register->send_otp($user_details->mobile_number, $message);
                            }

...
class Leaders_model extends CI_Model {
    ...
    public function get_all_leaders_details($offset, $no_of_rows, $search_params){
        ...
        $sql = "SELECT id,full_name,full_name_hindi,is_feature,total_vote 
                FROM leaders_master 
                WHERE is_active = 1";
    ...
    public function get_all_featured_leaders(){
        $sql = "SELECT id,full_name,full_name_hindi,image_path,is_feature,total_vote 
                FROM leaders_master 
    ...
    public function getLeaderInfo($params){
        if(!empty($params)){
            $sql = "SELECT id,full_name,full_name_hindi,image_path 
                    FROM leaders_master 
                    WHERE id = {$params['id']}";
            $query = $this->db->query($sql);        
    ...
    public function get_top_leaders(){
        $sql = "SELECT id,full_name,full_name_hindi,total_vote 
                FROM leaders_master 
                WHERE is_feature = 1 
	...
	public function get_top_six_leaders(){
        $sql = "SELECT id,full_name,full_name_hindi,total_vote 
                FROM leaders_master 
                WHERE is_feature = 1 
                ORDER BY total_vote DESC LIMIT 6";
        $query = $this->db->query($sql);        
        $top_leaders = $query->result_array();        
        return $top_leaders; 
    }
    ...
    public function get_all_other_leaders(){
        $sql = "SELECT id,full_name,full_name_hindi 
                FROM leaders_master 
    ...
    public function add_new_leader($params){
        $sql = "SELECT id FROM leaders_master WHERE full_name = '{$params['full_name']}' OR full_name_hindi = '{$params['full_name']}'";
		//$sql = "SELECT id FROM leaders_master WHERE full_name LIKE '%{$params['full_name']}%' OR `full_name_hindi` LIKE '%{$params['full_name']}%'";
        $query = $this->db->query($sql);   
    ...
    public function add_new_leader_api($params){
        /** PAGE VIEWS UPDATE LOGIC BY VICTOR AND PRADIP */
        $user_ip = $_SERVER['REMOTE_ADDR'];
        $page = 'https://www.indianpac.com/naf/vote/';
        ...
        if(!empty($leader_id)){
            ...
            $data['leader_id']   = $leader_id['id'];
            $data['is_new']      = 0;
            $data['session_id']  = $_GET['u_id'];
            $data['user_id']     = $params['added_by_user_id'];
            $data['user_ip']     = $_SERVER['REMOTE_ADDR'];
            
             $data['referral_code'] = "";
             $data['referral_owner_id'] = "";
            ...
            $this->db->trans_start();
            $this->db->insert('user_leader_vote_api', $data);
            ...
            $data = array();
            $data['name'] = $params['full_name'];
            $data['added_by_session_id'] = $_GET['u_id'];
            $data['added_by_user_id'] = $params['added_by_user_id'];
            $data['is_active']   = 1;
            $data['referral_code'] = "";
            $data['referral_owner_id'] = "";
            ...
            $data['created_at']  = date("Y-m-d H:i:s");
            $data['modified_at'] = date("Y-m-d H:i:s"); 
            $this->db->insert('new_leaders_api', $data);
            //$leader_id = $this->db->insert_id(); 
            return "Y";   
        }
    }
    ...
    public function getSelectedLeaderSummary($params){
        ...
        $sql = "SELECT ul.leader_id,l.full_name,l.full_name_hindi 
	...
	public function getnominatedleaders(){
		$return = array();
		$this->db->cache_off();
		$sql = "SELECT *
                FROM leaders_nominated_2nd order by leader_nominated";
        $query = $this->db->query($sql);
        $result =  $query->result_array();
		$return=$result;
		return $return;
